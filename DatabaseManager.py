...
class DatabaseManager:
    ...
    def get_machines(self,
        ...
        return {
            'count': results.count(),
            'data': helpr.clean_for_read(results_as_list)
        }

    def _map_history_to_machines(self, machines):
        for machine in machines:
            history = self.get_history(machine['serialNumber'], None, None,
                                       None, None)
            machine['historyCount'] = history['count']
        return machines

    def get_machines_by_property(self, property, limit, last_batch_fetched,
                                 sort_by, sort_order):
        machines = self.db.machines
        ...
        results = machines.find({search_type: search_val},
                                self.query_projections)
        if sort_by is not None:
            results = results.sort([('score', {
                '$meta': 'textScore'
            }), (sort_by, helpr.get_sort_order(sort_order))])
        if limit is not None:
            results = results.skip(int(last_batch_fetched)).limit(int(limit))
        results_as_list = list(results)
        self._map_history_to_machines(results_as_list)
    ...
    def get_machine_by_id(self, id):
        machines = self.db.machines
        return helpr.clean_single_object_for_read(
            machines.find_one({'serialNumber': id}, self.query_projections)
            or {})

    def get_machines_by_ids(self, ids):
        machines = self.db.machines
        results = machines.find({'serialNumber': {
            '$in': ids
        }}, self.query_projections)
        return {
            'count': results.count(),
    ...
    def update_machine(self, id, new_values):
        ...
        if 'attachment' in machine and machine['attachment']:
            ...
            if 'attachment' not in new_values or not new_values['attachment']:
                ...
                self.delete_attachment(current_attachment_id)
        return machines.update_one({'serialNumber': id},
                                   {'$set': helpr.clean_for_write(new_values)})
    ...
    def insert_attachment(self, id, attachment, filename):
        attachment_id = self.gfs.put(attachment,
                                     parent_id=id,
                                     filename=filename)
    ...
    def get_history(self, machine_id, limit, last_batch_fetched, sort_by,
                    sort_order):
        history = self.db.maintenance
        results = history.find({'serialNumber': machine_id},
                               self.query_projections)
        if sort_by is not None:
            results = results.sort(sort_by, helpr.get_sort_order(sort_order))
        if limit is not None:
    ...
    def get_history_by_property(self, machine_id, property, limit,
                                last_batch_fetched, sort_by, sort_order):
        history = self.db.maintenance
        if property.startswith('"') and property.endswith('"'):
            search_type = '$text'
            search_val = {'$search': property}
    ...
    def get_history_by_id(self, id):
        history = self.db.maintenance
        return helpr.clean_single_object_for_read(
            history.find_one({'_id': ObjectId(id)}, self.query_projections)
            or {})
    ...
    def update_history(self, id, new_values):
        ...
        if 'attachment' in record and record['attachment']:
            ...
            if 'attachment' not in new_values or not new_values['attachment']:
                ...
                self.delete_attachment(current_attachment_id)
        return history.update_one({'_id': ObjectId(id)},
                                  {'$set': helpr.clean_for_write(new_values)})
    ...
    def create_user(self, credentials):
        ...
        if users.find_one({'username': credentials['username']}):
            return False
        salt = os.urandom(16)
        hashed_pwd = helpr.hash(credentials['password'], salt)
        credentials['password'] = hashed_pwd
        credentials['salt'] = salt
        return users.insert_one(credentials)

    def confirm_user(self, credentials):
        users = self.db.users
        user = users.find_one({'username': credentials['username']})
        if not user:
            return False
        hashed_pwd = helpr.hash(credentials['password'], user['salt'])
